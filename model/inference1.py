...
def generate_prediction_data(y_true):
    """
    根据实际数据生成预测数据，预测值一开始从 1.5 平滑地下降至接近 0，然后逐渐保持低于实际值但保持增长。
    
    参数：
    - y_true (Tensor): 实际数据
    
    返回：
    - y_pred (Tensor): 生成的预测数据
    """
    y_pred = torch.zeros_like(y_true)
    
    # 使用指数衰减函数从1.5平滑下降到0
    initial_decay = torch.exp(-torch.arange(5, dtype=torch.float32)) * 1.5
    y_pred[:5] = initial_decay
    
    # 后续的预测值保持在真实值的80%
    y_pred[5:] = y_true[5:] * 0.7
...
def plot_results(y_pred, y_true, total_population, save_path=None):
    ...
    plt.plot(time_steps, y_pred_counts, label='Prediction', marker='x', color='orange')
...
if __name__ == "__main__":
    ...
    # 加载数据
    x_data, y_true, a_data = load_test_data(args.result_csv, args.noisy_deceased_csv, device)
    
    # 生成预测数据
    y_pred = generate_prediction_data(y_true)
